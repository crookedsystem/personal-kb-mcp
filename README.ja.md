...
- **Frontmatter:** `title`、`created`、`updated`、`type`、`tags`、`sources` は必須で、`created` と `updated` は秒まで含み末尾に `Z` が付いた UTC ISO datetime である必要があります（`YYYY-MM-DDTHH:MM:SSZ`）。`confidence`（high/medium/low）と `contested`（true/false）は任意。
