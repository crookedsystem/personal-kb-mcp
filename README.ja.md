...
- **Frontmatter:** `title`、`created`、`updated`、`type`、`tags`、`sources` は必須で、`created` と `updated` は秒まで含む ISO datetime（`YYYY-MM-DDTHH:MM:SS`）である必要があります。`confidence`（high/medium/low）と `contested`（true/false）は任意。
