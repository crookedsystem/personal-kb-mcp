...
- **Frontmatter:** `title`、`created`、`updated`、`type`、`tags`、`sources` は必須で、`created` と `updated` は秒まで含む ISO datetime である必要があり、UTC に正規化され末尾に `Z` を付けて保存されます（`YYYY-MM-DDTHH:MM:SSZ`）。naive な値は UTC とみなされ、offset 付き入力は UTC に変換されます。`confidence`（high/medium/low）と `contested`（true/false）は任意。
