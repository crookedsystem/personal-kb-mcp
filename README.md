...
- **Frontmatter:** `title`, `created`, `updated`, `type`, `tags`, `sources` are required; `created` and `updated` must be ISO datetimes with seconds (`YYYY-MM-DDTHH:MM:SS`); `confidence` (high/medium/low) and `contested` (true/false) are optional.
