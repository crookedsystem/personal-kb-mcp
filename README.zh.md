...
- **Frontmatter:** `title`、`created`、`updated`、`type`、`tags`、`sources` 必填；`created` 和 `updated` 必须是包含秒的 ISO datetime，会被规范化为 UTC 并以结尾 `Z` 存储（`YYYY-MM-DDTHH:MM:SSZ`）——naive 值视为 UTC，带 offset 的输入会转换为 UTC；`confidence`（high/medium/low）与 `contested`（true/false）可选。
