...
- **Frontmatter:** `title`、`created`、`updated`、`type`、`tags`、`sources` 必填；`created` 和 `updated` 必须是包含秒的 ISO datetime（`YYYY-MM-DDTHH:MM:SS`）；`confidence`（high/medium/low）与 `contested`（true/false）可选。
